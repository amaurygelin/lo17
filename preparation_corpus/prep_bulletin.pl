...
# The regular expressions that let us find elements in the file are proved to be sound (see documentation.txt)
...
while(<$fd>) { 
    ...
    if($_ =~ /<p class="style96"><span class="style95">/){
        $flag_text = 1; # beginning of text
    }

    if($flag_text && $_ =~ /<\/p>/) {
        $flag_text = 0; # end of text
    }
    ...
    if($flag_text && $_ !=~ /\\n/) {
        ...
        } elsif($flag_image == 0 && $_ =~ /<span class="style21"><strong>(.*)<\/strong>/){
            print($logFile "KO - Problem with file $ARGV[0] : legend without image : $1\n");
    ...
    if($_ =~ /<p class="style44"><span class="style85">(.*)<\/span><\/p>/){
       $contact = $1 ;
    }
}
...
if ($fichier eq "" || $numero eq "" || $date eq "" || $rubrique eq "" || $titre eq "" || $texte eq "" || $contact eq ""){
    print($logFile "KO - Missing information in file $ARGV[0] \n");
} else {
